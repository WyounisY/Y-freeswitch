...
static char *vad_serialize_json(switch_vad_t *vad, int callstatus)
{
	cJSON *pJson = NULL;
	char *writebuf = NULL;
...
static char *vad_parse_Json(char *readbuf, switch_vad_t *vad)
{
	cJSON *pJson = NULL;
	cJSON *pSub = NULL;
...
static int create_client(switch_vad_t *g_vad)
{
...
switch_vad_process(switch_vad_t *vad, int16_t *data, unsigned int samples)
{
...
static switch_bool_t vad_audio_callback(switch_media_bug_t *bug, void *user_data, switch_abc_type_t type)
{
	...
	case SWITCH_ABC_TYPE_READ_REPLACE:
		...
		switch_vad_process(vad, linear_frame->data, linear_len / 2);
