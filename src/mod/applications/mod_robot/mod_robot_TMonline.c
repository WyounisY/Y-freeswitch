...
#define TM_PRIVATE "_tianmao_"		  // tianmao模块哈希key值
#define TM_XML_CONFIG "tianmao.conf" // 配置文件名
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_tianmao_shutdown);
SWITCH_MODULE_LOAD_FUNCTION(mod_tianmao_load);
SWITCH_MODULE_DEFINITION(mod_tianmao, mod_tianmao_load, mod_tianmao_shutdown, NULL);
SWITCH_STANDARD_APP(tianmao_start_function);
 ...
 * @param json_data 输入的 JSON 数据。
 * @return 如果 istianmao 为 true 返回 true，否则返回 false。
 */
static switch_bool_t get_istianmao(const char *json_data)
{
	cJSON *istianmao_item;
	switch_bool_t result;
	cJSON *json = parse_json_data(json_data);
	if (!json) {
	...
	istianmao_item = cJSON_GetObjectItem(json, "is_vad");
	if (!istianmao_item) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "is_tianmao 字段在 JSON 数据中不存在。\n");
		cJSON_Delete(json);
		return SWITCH_FALSE; // 默认返回 false，字段不存在
	}

	result = cJSON_IsTrue(istianmao_item);

	cJSON_Delete(json);
	return result;
}

static char *tianmao_serialize_json(switch_tianmao_docker_t *tianmao, int callstatus)
{
	cJSON *pJson = NULL;
	char *writebuf = NULL;
	switch_channel_t *channel = switch_core_session_get_channel(tianmao->session);
	...
	cJSON_AddStringToObject(pJson, "uuid", tianmao->uuid);
...
/**
 * 创建客户端并连接到服务器。
 * @param g_tianmao 指向 switch_tianmao_docker_t 结构体的指针。
 * @return 成功返回 0，失败返回负值。
 */
static int create_client(switch_tianmao_docker_t *g_tianmao)
{
	struct sockaddr_in SockAddr = {0};
	int ret = -1;

	g_tianmao->cfd = socket(AF_INET, SOCK_STREAM, 0);
	if (g_tianmao->cfd < 0) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Failed to create socket\n");
		return g_tianmao->cfd;
	}
	...
	if (0 > (ret = connect(g_tianmao->cfd, (struct sockaddr *)&SockAddr, sizeof(SockAddr)))) {
 ...
 * 关闭 TM Docker 函数
 *
 * @param tianmao 输入的指向需要初始化的 switch_tianmao_docker_t 结构体的指针。
 * @return 返回一个 switch_bool_t 值，表示初始化是否成功。
 */
static switch_bool_t switch_tianmao_docker_close(switch_tianmao_docker_t *tianmao)
{
	int ret = -1;
	switch_core_session_t *session = tianmao->session;
	ret = send(tianmao->cfd, "call_end", strlen("call_end"), 0);
	if (ret <= 0) {
	...
	// switch_thread_join(&st, tianmao->thread);

	close(tianmao->cfd);

	switch_mutex_destroy(tianmao->audio_mutex);

	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
					  "Stopping TM detection for audio stream\n");
	return SWITCH_TRUE;
}
...
{
	long headerSize = 0;
	switch_tianmao_docker_t *tianmao = (switch_tianmao_docker_t *)user_data;
	char readbuf[360] = {0};
	int ret = -1;
	void *dummy;
	const char *tts_path;
	switch_bool_t istianmao;
	switch_channel_t *channel = switch_core_session_get_channel(tianmao->session);

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "----------------RecvPthread start !!----------------\n");

	while (switch_channel_ready(channel)) {
		ret = recv(tianmao->cfd, readbuf, sizeof(readbuf), 0);
		if (ret < 0) {
			...
			tianmao->log_flag = TRUE;

			// 判断当前是否是接收到了模型的tianmao信息，表示有人说话了
			istianmao = get_istianmao(readbuf);
			if (!istianmao) {

				switch_mutex_lock(tianmao->audio_mutex);
				// 清空 audio_queue
				while (switch_queue_size(tianmao->audio_queue) >= 1) {
					// switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "需要打断,正在清理audio_queue队列\n");

					if (switch_queue_trypop(tianmao->audio_queue, &dummy) != SWITCH_STATUS_SUCCESS) {
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "trypop audio_queue 失败\n");
					}
				}
				switch_mutex_unlock(tianmao->audio_mutex);
				...
				if (tts_path && strlen(tts_path) > 0) {
					...
					if (fd) {
						...
						while ((bytes_read = read(fd, filebuf, sizeof(filebuf))) > 0) {
							...
							while (switch_queue_trypush(tianmao->audio_queue, buffer_copy) != SWITCH_STATUS_SUCCESS) {
								if (tianmao->retry_count >= 3) {
									tianmao->retry_count = 0;
									break;
								}
								tianmao->retry_count++;
				...
				switch_mutex_lock(tianmao->audio_mutex);
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "iscontiue_flag 在线程中上锁\n");
				tianmao->iscontiue_flag = FALSE;
				switch_mutex_unlock(tianmao->audio_mutex);
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "iscontiue_flag 在线程中解锁\n");
				// 判断 flag 是否为 "call_end"
				if (get_flag_and_check(readbuf)) {
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "线程接收到call_end标志\n");
					tianmao->is_playback_end = TRUE;
					break;
				}
			}
		...
		if (tianmao->audio_pthread_exit) { break; }
	}
	tianmao->pthread_exit = TRUE;
...
static void *SWITCH_THREAD_FUNC AudioProcessPthread(switch_thread_t *thread, void *user_data)
{
	...
	while (switch_channel_ready(channel)) {
		...
		// 线程退出不再发送音频数据
		if (!tianmao->pthread_exit) {
			ret = send(tianmao->cfd, read_frame->data, read_frame->datalen, 0);
			if (ret < 0) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
								  "when talking send socket data fail errno is :%s!!\n", strerror(errno));
				switch_channel_hangup(channel, SWITCH_CAUSE_NORMAL_CLEARING);
			} else if (ret > 0) {
				if (tianmao->log_flag == TRUE) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
									  "send len is :%d everytime!!\n", ret);
					tianmao->log_flag = FALSE;
				}
			} else {
		...
		switch_mutex_lock(tianmao->audio_mutex);
		if (tianmao->iscontiue_flag) {
			if (tianmao->log_flag == TRUE) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "获取音频队列的开关打开 \n");
			}
			audio_size = switch_queue_size(tianmao->audio_queue);
			if (audio_size >= 1) {
				// 播放开始标志
				tianmao->is_playback_end = FALSE;
				if (tianmao->log_flag == TRUE) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
									  "音频队列的数据大于等于1 %d \n", audio_size);
					tianmao->log_flag = FALSE;
				}
				switch_queue_pop(tianmao->audio_queue, &pop);
				// switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "弹出的数据是%s\n", (char *)pop);

				// 复制pop指向的内容到data
				memcpy(read_frame->data, pop, 320);

				switch_core_session_write_frame(tianmao->session, read_frame, SWITCH_IO_FLAG_NONE, 0);
				// switch_core_media_bug_set_write_replace_frame(bug, linear_frame);

				// 将pop清空
				pop = NULL;
			} else {
				ret = send(tianmao->cfd, tianmao->sendbuf, strlen(tianmao->sendbuf), 0);
				if (ret <= 0) {
		...
		if (tianmao->pthread_exit && tianmao->is_playback_end) {
	...
	tianmao->audio_pthread_exit = TRUE;
 ...
 * @return 返回一个 switch_bool_t 值，表示初始化是否成功。
 */
static switch_bool_t switch_tianmao_docker_init(switch_tianmao_docker_t *tianmao)
{
	...
	tianmao->log_flag = TRUE;
	tianmao->uuid = NULL;
	tianmao->pthread_exit = FALSE;
	tianmao->iscontiue_flag = FALSE;
	tianmao->is_playback_end = FALSE;
	tianmao->audio_pthread_exit = FALSE;
	tianmao->retry_count = 0;

	session = tianmao->session;
	channel = switch_core_session_get_channel(session);

	if (NULL == tianmao) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "tianmao为NULL");
		return SWITCH_FALSE;
	}

	strcpy(tianmao->sendbuf, "playback_end");
	switch_queue_create(&tianmao->audio_queue, MAX_AUDIO_QUEUE_LEN, switch_core_session_get_pool(tianmao->session));

	// 初始化
	if ((ret = create_client(tianmao)) == -1) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "create_client fail\n");
		return SWITCH_FALSE;
	}

	tianmao->uuid = switch_core_session_get_uuid(session);
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "这通电话的uuid: %s \n", tianmao->uuid);

	serialize_json = tianmao_serialize_json(tianmao, 1);
	if (NULL == serialize_json) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "tianmao_serialize_json fail!!\n");
	} else {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
						  "tianmao_serialize_json success writebuf is %s\n", serialize_json);
	}

	ret = send(tianmao->cfd, serialize_json, strlen(serialize_json), 0);
	if (ret <= 0) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
	...
	if (ret <= 0) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
						  "recv()  empty data tianmao no catch :%s!!\n", strerror(errno));
		switch_channel_hangup(channel, SWITCH_CAUSE_NORMAL_CLEARING);
	} else {
	...
	// 打开文件
	// tianmao->test_file = open("/usr/local/freeswitch/recordings/test.raw", O_WRONLY | O_CREAT | O_TRUNC, 0644);
	// if (tianmao->test_file == -1) {
	// 	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Failed to open file ");
	// 	return SWITCH_FALSE;
	// }

	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
					  "Starting TM detection for audio stream  ");

	tianmao->pool = switch_core_session_get_pool(session);

	// 在初始化代码中创建互斥锁
	switch_mutex_init(&tianmao->audio_mutex, SWITCH_MUTEX_NESTED, tianmao->pool);

	switch_threadattr_create(&thd_attr, tianmao->pool);
	switch_threadattr_detach_set(thd_attr, 1);
	switch_threadattr_stacksize_set(thd_attr, SWITCH_THREAD_STACKSIZE);
	switch_thread_create(&tianmao->thread, thd_attr, RecvPthread, tianmao, tianmao->pool);

	switch_threadattr_create(&thd_farm, tianmao->pool);
	switch_threadattr_detach_set(thd_farm, 1);
	switch_threadattr_stacksize_set(thd_farm, SWITCH_THREAD_STACKSIZE);
	switch_thread_create(&tianmao->audio_thread, thd_farm, AudioProcessPthread, tianmao, tianmao->pool);
 ...
 */
// static switch_bool_t tianmao_audio_callback(switch_media_bug_t *bug, void *user_data, switch_abc_type_t type)
// {
// 	switch_tianmao_docker_t *tianmao = (switch_tianmao_docker_t *)user_data;
// 	switch_core_session_t *session = tianmao->session;
...
SWITCH_MODULE_LOAD_FUNCTION(mod_tianmao_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	SWITCH_ADD_APP(app_interface, "tianmao", "Voice activity detection", "Freeswitch's TM", tianmao_start_function,
				   "[start|stop]", SAF_NONE);

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, " tianmao_load successful...\n");

	return SWITCH_STATUS_SUCCESS;
}

SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_tianmao_shutdown) { return SWITCH_STATUS_SUCCESS; }

SWITCH_STANDARD_APP(tianmao_start_function)
{
	switch_channel_t *channel = switch_core_session_get_channel(session);
	switch_tianmao_docker_t *s_tianmao = NULL;
	switch_codec_implementation_t imp = {0};
	switch_bool_t ret;

	if (!zstr(data)) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "TM input parameter %s\n", data);
	}

	if ((s_tianmao = (switch_tianmao_docker_t *)switch_channel_get_private(channel, TM_PRIVATE))) {
		if (!zstr(data) && !strcasecmp(data, "stop")) {
			switch_channel_set_private(channel, TM_PRIVATE, NULL);
			if (s_tianmao->read_bug) {
				switch_core_media_bug_remove(session, &s_tianmao->read_bug);
				s_tianmao->read_bug = NULL;
				switch_core_session_reset(session, SWITCH_TRUE, SWITCH_TRUE);
			}
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Stopped TM detection\n");
		} else {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING,
							  "Cannot run tianmao detection 2 times on the same session!\n");
		}
		return;
	}

	s_tianmao = switch_core_session_alloc(session, sizeof(*s_tianmao));
	switch_assert(s_tianmao);
	memset(s_tianmao, 0, sizeof(*s_tianmao));
	s_tianmao->session = session;
	...
	ret = switch_tianmao_docker_init(s_tianmao);
	if (!ret) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "程序初始化失败，结束通话\n");
		return;
	}

	switch_channel_set_private(channel, TM_PRIVATE, s_tianmao);

	while (!s_tianmao->audio_pthread_exit) { switch_sleep(20 * 1000); }
	ret = switch_tianmao_docker_close(s_tianmao);
	if (!ret) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "程序初始化失败，结束通话\n");
		return;
	}
